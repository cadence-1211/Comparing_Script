...
def extract_value(value_str, expect_numeric):
    if expect_numeric:
        if re.fullmatch(r"[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?", value_str):
            try:
                ...
                pass
        return None  # invalid numeric
    return value_str  # treat as string

def parse_file_with_mmap(file_path, inst_cols, value_col, expect_numeric):
    data = {}
    instances_set = set()
    with open(file_path, "rb") as f:
        ...
        while i < len(lines) - 1:
            ...
            if line1 and len(line2.split()) >= 8:
                combined = line1 + b" " + line2
                parts = combined.strip().split()
                try:
                    key = tuple(parts[j].decode("utf-8", errors="ignore").strip() for j in inst_cols)
                    val_raw = parts[value_col].decode("utf-8", errors="ignore").strip()
                    val_parsed = extract_value(val_raw, expect_numeric)
...
def write_comparison_csv(file1_name, file2_name, data1, data2, matched, col_name1, col_name2, expect_numeric):
    with open("comparison.csv", "w", newline="") as csvfile:
        ...
        for inst in matched:
            ...
            if expect_numeric and isinstance(val1, float) and isinstance(val2, float):
                diff = val1 - val2
                deviation = (abs(diff) / abs(val2)) * 100 if val2 != 0 else float('inf')
...
def main():
    parser = argparse.ArgumentParser(description="Compare two files with flexible multi-column keys")
    parser.add_argument("--file1", help="Path to first file")
    parser.add_argument("--instcol1", help="Comma-separated match column indexes for file1")
    ...
    parser.add_argument("--instcol2", help="Comma-separated match column indexes for file2")
    ...
    if not args.instcol1:
        args.instcol1 = input("Enter comma-separated match column indexes for file1: ")
    ...
    if not args.instcol2:
        args.instcol2 = input("Enter comma-separated match column indexes for file2: ")
    ...
    if len(instcol1) != len(instcol2):
        print("❌ Match column count mismatch!")
        sys.exit(1)

    ans = input("Is the value column a string to be matched (y/n)? ").lower()
    expect_numeric = False if ans == 'y' else True
    ...
    with multiprocessing.Pool(2) as pool:
        results = pool.map(parse_file_worker, [
            (args.file1, instcol1, args.valcol1, expect_numeric),
            (args.file2, instcol2, args.valcol2, expect_numeric)
        ])
    ...
    write_comparison_csv(file1_name, file2_name, data1, data2, matched, col_name1, col_name2, expect_numeric)
